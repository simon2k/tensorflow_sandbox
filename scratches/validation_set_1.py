...
model = get_regularized_model(train_dataset, wd=1e-5, rate=.3)
compile_model(model)
...
model.evaluate(test_dataset, test_targets, callbacks=[testing_callback])  # no dropout - also in model.predict
...
prediction = model.predict(values[np.newaxis, ...], callbacks=[prediction_callback])[0]
